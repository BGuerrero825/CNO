...
//
void PrintAddress(const char* desc, SOCKADDR* sa, size_t salen)
{
...
int FormatAddress(SOCKADDR* sa, int salen, char* addrbuf, int addrbuflen)
{
    ...
    if (strncmp(serv, "0", 1) != 0)
    {
        if (sa->sa_family == AF_INET)
            _snprintf_s(addrbuf, addrbuflen, (size_t)addrbuflen - 1, "%s:%s", host, serv);
    ...
    {
        _snprintf_s(addrbuf, addrbuflen, (size_t)addrbuflen - 1, "%s", host);
    }
...
//    it is a string listeral address or a hostname. If 'addr' is NULL, it will 
//    bind to the default local interface.
//
struct addrinfo* ResolveAddress(const char* addr, const char* port, int af, int type, int proto)
{
    ...
    if (rc != 0)
    {
        printf("Invalid address %s:%s, getaddrinfo failed: %d\n", addr, port, rc);
        return NULL;
    }
    return res;
}
...
int ReverseLookup(SOCKADDR* sa, int salen, char* buf, int buflen)
{
    ...
    strncpy_s(buf, buflen, host, (size_t)buflen - 1);
